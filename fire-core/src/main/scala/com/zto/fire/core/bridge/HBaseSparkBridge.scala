...
import com.zto.fire.common.util.{GlobalConstants, HBaseUtils, ValueUtils}
...
object HBaseSparkBridge extends HBaseOper with Logging {
   ...
   * @return
   */
  def hbaseOperScanTable[T <: HBaseBaseBean[T] : ClassTag](spark: SparkSession, tableName: String, scan: Scan, clazz: Class[T], cacheTable: Boolean = true, multiVersion: Boolean = false, versions: Int = Integer.MAX_VALUE): DataFrame = {
    val df = this.hbaseOperScanDF[T](spark, tableName, scan, clazz, multiVersion, versions)
   ...
   * @return
   */
  def hbaseOperScanTable2[T <: HBaseBaseBean[T] : ClassTag](spark: SparkSession, tableName: String, startRow: String, stopRow: String, clazz: Class[T], cacheTable: Boolean = true, multiVersion: Boolean = false, versions: Int = Integer.MAX_VALUE): DataFrame = {
    val df = this.hbaseOperScanDF2[T](spark, tableName, startRow, stopRow, clazz, multiVersion, versions)
    if (cacheTable && ValueUtils.isNotEmpty(df)) df.createOrReplaceTempViewCache(tableName) else df.createOrReplaceTempView(tableName)
    df
  }
   ...
   * @return
   */
  def hbaseHadoopScanTable[T <: HBaseBaseBean[T] : ClassTag](spark: SparkSession, tableName: String, scan: Scan, clazz: Class[T], cacheTable: Boolean = true, multiVersion: Boolean = false, versions: Int = Integer.MAX_VALUE): DataFrame = {
    val df = this.hbaseHadoopScanDF[T](spark, tableName, scan, clazz, multiVersion, versions)
    if (cacheTable && ValueUtils.isNotEmpty(df)) df.createOrReplaceTempViewCache(tableName) else df.createOrReplaceTempView(tableName)
    df
  }
   ...
   * @return
   */
  def hbaseHadoopScanTable2[T <: HBaseBaseBean[T] : ClassTag](spark: SparkSession, tableName: String, startRow: String, stopRow: String, clazz: Class[T], cacheTable: Boolean = true, multiVersion: Boolean = false, versions: Int = Integer.MAX_VALUE): DataFrame = {
    val df = this.hbaseHadoopScanDF2[T](spark, tableName, startRow, stopRow, clazz, multiVersion, versions)
  ...
  /**
   * 通过RDD[String]批量获取对应的数据，并注册成Spark临时表（可获取历史版本的记录）
   *
   * @param rowKeyRDD
   * rdd中存放了待查询的rowKey集合
   * @param tableName
   ...
   * @return
   */
  def hbaseOperGetTable[T <: HBaseBaseBean[T] : ClassTag](tableName: String, rowKeyRDD: RDD[String], clazz: Class[T], cacheTable: Boolean = true, multiVersion: Boolean = false, versions: Int = Integer.MAX_VALUE): DataFrame = {
    val df = this.hbaseOperGetDF[T](tableName, rowKeyRDD, clazz, multiVersion, versions)
    if (cacheTable && ValueUtils.isNotEmpty(df)) df.createOrReplaceTempViewCache(tableName) else df.createOrReplaceTempView(tableName)
    df
  }
