...
public class HBaseOper {
    ...
    private static <T extends HBaseBaseBean> Map<String, Field> getFieldNameMap(Class<T> clazz) {
        if (clazz != null) {
            ...
            Map<String, Field> fieldMap = new HashMap<String, Field>(allFields.size());
    ...
    public static <T extends HBaseBaseBean> T hbaseRow2Bean(Result rs, Class<T> clazz) {
        ...
        try {
            ...
            Field idField = clazz.getField("rowKey");
            if (idField == null) {
    ...
    public static <T extends HBaseBaseBean> List<T> hbaseRow2Bean(Result[] rsArr, Class<T> clazz) {
        ...
        List<T> objList = new ArrayList<T>(rsArr.length);
        for (Result rs : rsArr) {
            if (rs.isEmpty()) {
                continue;
            }
     ...
     * @param tableName 表名
     */
    public static void disable(String tableName) {
        Admin admin = null;
        try {
            admin = connection.getAdmin();
            TableName tbName = TableName.valueOf(tableName);
    ...
    public static <T extends HBaseBaseBean> Put convert2Put(T obj, boolean insertEmpty) {
        if (obj != null) {
            try {
                ...
                // 获取RowKey字段中的值
                Field rowKeyField = clazz.getField("rowKey");
                rowKeyField.setAccessible(true);
                Object rowKeyObj = rowKeyField.get(obj);
                if (rowKeyObj == null) {
                    Method method = clazz.getMethod("buildRowKey");
    ...
    /**
     * 获取多条数据的rowkey集合，组装成Spark.Row形式
     *
     * @param tableName 表名
     * @param getList   HBase的get对象实例
     * @return rowkey集合
     */
    /*public static ListBuffer<org.apache.spark.sql.Row> getRowkeyList(String tableName, ListBuffer<Get> getList) {
        Result[] rsArr = get(tableName, scala.collection.JavaConversions.asJavaList(getList));
        ListBuffer<org.apache.spark.sql.Row> rowListBuffer = new ListBuffer<>();
        try {
            byte[] row = null;
            for (Result rs : rsArr) {
                row = rs.getRow();
                if (row != null) {
                    rowListBuffer.$plus$eq(RowFactory.create(new String(row)));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return rowListBuffer;
    }*/
