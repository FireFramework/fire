...
public class Hive {
   ...
   * @throws HiveException
   * @see HiveMetaStoreClient#dropDatabase(String)
   */
  ...
  public Map<Map<String, String>, Partition> loadDynamicPartitions(Path loadPath,
    ...
    try {
      ...
      for (FileStatus s : leafStatus) {
        ...
        if (s.isDir() &&
                !conf.getBoolVar(ConfVars.HIVE_HADOOP_SUPPORTS_SUBDIRECTORIES)) {
      ...
      if (validPartitions.size() > conf.getIntVar(ConfVars.DYNAMICPARTITIONMAXPARTS)) {
  ...
  public Map<Map<String, String>, Partition> loadDynamicPartitions(Path loadPath,
    ...
    try {
      ...
      if (isAcid) {
        ...
        metaStoreClient.addDynamicPartitions(0L, tbl.getDbName(), tbl.getTableName(), partNames);
      }
      return partitionsMap;
    } catch (IOException e) {
      throw new HiveException(e);
    ...
    try {
      ...
      for (FileStatus src : srcs) {
        ...
        for (FileStatus item : items) {
          ...
          if (!conf.getBoolVar(ConfVars.HIVE_HADOOP_SUPPORTS_SUBDIRECTORIES) &&
                  !HiveConf.getVar(conf, ConfVars.STAGINGDIR).equals(itemSource.getName()) &&
                  item.isDir()) {
