...
object JdbcUtils extends Logging {
  ...
  /**
    * Returns an Merge SQL statement for inserting a row into the target table via JDBC conn.
    */
  def getMergeStatement(
    ...
    val duplicateStatement = rddSchema.fields.map(x => x.name).map(f => s"$f=?").mkString(",")
    val placeholders = rddSchema.fields.map(_ => "?").mkString(",")
    println(s"INSERT INTO $table ($columns) VALUES ($placeholders) ON DUPLICATE KEY UPDATE $duplicateStatement")
    s"INSERT INTO $table ($columns) VALUES ($placeholders) ON DUPLICATE KEY UPDATE $duplicateStatement"
  }
  ...
  private def makeSetter(
                          conn: Connection,
                          dialect: JdbcDialect,
                          dataType: DataType, step: Int = 0): JDBCValueSetter = dataType match {
    case IntegerType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setInt(pos + 1 + step, row.getInt(pos))

    case LongType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setLong(pos + 1 + step, row.getLong(pos))

    case DoubleType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setDouble(pos + 1 + step, row.getDouble(pos))

    case FloatType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setFloat(pos + 1 + step, row.getFloat(pos))

    case ShortType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setInt(pos + 1 + step, row.getShort(pos))

    case ByteType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setInt(pos + 1 + step, row.getByte(pos))

    case BooleanType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setBoolean(pos + 1 + step, row.getBoolean(pos))

    case StringType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setString(pos + 1 + step, row.getString(pos))

    case BinaryType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setBytes(pos + 1 + step, row.getAs[Array[Byte]](pos))

    case TimestampType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setTimestamp(pos + 1 + step, row.getAs[java.sql.Timestamp](pos))

    case DateType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setDate(pos + 1 + step, row.getAs[java.sql.Date](pos))

    case t: DecimalType =>
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        stmt.setBigDecimal(pos + 1 + step, row.getDecimal(pos))
    ...
    case ArrayType(et, _) =>
      ...
      (stmt: PreparedStatement, row: Row, pos: Int) =>
        ...
        stmt.setArray(pos + 1 + step, array)
  ...
  /**
    * Merge a partition of a DataFrame to the JDBC database.  This is done in
    * a single database transaction (unless isolation level is "NONE")
    * in order to avoid repeatedly inserting data as much as possible.
    *
    * It is still theoretically possible for rows in a DataFrame to be
  ...
  def mergePartition(
    ...
    try {
      ...
      val stmt = conn.prepareStatement(mergeStmt)
      val setters = rddSchema.fields.map(f => makeSetter(conn, dialect, f.dataType))
      val nullTypes = rddSchema.fields.map(f => getJdbcType(f.dataType, dialect).jdbcNullType)
      val numFields = rddSchema.fields.length
      val settersDuplicate = rddSchema.fields.map(f => makeSetter(conn, dialect, f.dataType, numFields))
      ...
      try {
        ...
        while (iterator.hasNext) {
          ...
          while (i < numFields) {
            if (row.isNullAt(i)) {
              stmt.setNull(i + 1, nullTypes(i))
              stmt.setNull(i + numFields + 1, nullTypes(i))
            } else {
              setters(i).apply(stmt, row, i)
              settersDuplicate(i).apply(stmt, row, i)
            }
            i = i + 1
          }
          stmt.addBatch()
  ...
  /**
    * merge the RDD to the database in a single transaction.
    */
  def mergeTable(
                 df: DataFrame,
                 tableSchema: Option[StructType],
    ...
    val mergeStmt = getMergeStatement(table, rddSchema, tableSchema, isCaseSensitive, dialect)
    val repartitionedDF = options.numPartitions match {
      case Some(n) if n <= 0 => throw new IllegalArgumentException(
        s"Invalid value `$n` for parameter `${JDBCOptions.JDBC_NUM_PARTITIONS}` in table writing " +
          "via JDBC. The minimum value is 1.")
    ...
    repartitionedDF.rdd.foreachPartition(iterator => mergePartition(
      getConnection, table, iterator, rddSchema, mergeStmt, batchSize, dialect, isolationLevel)
    )
  }
