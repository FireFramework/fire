...
private[spark] class DirectKafkaInputDStream[K, V](
  ...
  private val offsetImmediateCommitEnable = context.sparkContext.getConf.getBoolean(
    "spark.streaming.kafka.offset.immediateCommit.enable", false)
  ...
  def commitAsync(offsetRanges: Array[OffsetRange], callback: OffsetCommitCallback): Unit = {
    ...
    if (offsetImmediateCommitEnable) {
      this.commitAll()
    }
  }
