...
public class KafkaSourceBuilder<OUT> {
    ...
    private void parseAndSetRequiredProperties() {
        ...
        // TODO: ------------ start：二次开发代码 --------------- //
        if (props.containsKey("kafka.force.autoCommit.enable") && Boolean.parseBoolean(
                props.getProperty("kafka.force.autoCommit.enable"))) {
            if (props.containsKey("kafka.force.autoCommit.Interval")) {
                props.setProperty(ConsumerConfig.AUTO_COMMIT_INTERVAL_MS_CONFIG, props.getProperty("kafka.force.autoCommit.Interval"));
            }
        } else {
