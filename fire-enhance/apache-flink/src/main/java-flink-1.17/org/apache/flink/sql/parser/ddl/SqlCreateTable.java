...
public class SqlCreateTable extends SqlCreate implements ExtendedSqlNode {
    ...
    public String[] fullTableName() {
        // TODO: ------------ start：二次开发代码 --------------- //
        // local模式下避免jar包冲突导致运行报错
        if (!OSUtils.isLinux()) {
            try {
                Field field = tableName.getClass().getField("names");
                Object names = field.get(tableName);

                if (names != null) {
                    Method toArray = names.getClass().getMethod("toArray");
                    Object[] arr = (Object[]) toArray.invoke(names);
