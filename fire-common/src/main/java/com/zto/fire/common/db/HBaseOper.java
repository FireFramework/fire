...
public class HBaseOper {
    ...
    public static void insert(String tableName, String rowKey, String family, String qualifier, String value) {
        if (isExists(tableName) && StringUtils.isNotBlank(rowKey)) {
            try {
                ...
                logger.error("数据写入hbase失败, cluster={} tableName={} rowKey={} family={} qualifier={} value={} exception={}", CLUSTER, tableName, rowKey, family, qualifier, value, ExceptionBus.stackTrace(e));
            }
        }
    }
    ...
    public static void insert(String tableName, Put put) {
        if (isExists(tableName) && put != null) {
            try {
                ...
                logger.error("数据写入hbase失败, cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
            }
        }
    }
    ...
    public static void insertPut(String tableName, List<Put> putList) {
        if (isExists(tableName) && putList != null && !putList.isEmpty()) {
            ...
            try {
                ...
                logger.error("数据写入hbase失败, cluster={} tableName={} size={} exception={}", CLUSTER, tableName, putList.size(), ExceptionBus.stackTrace(e));
            } finally {
                closeTable(table);
            }
        }
    ...
    public static Result get(String tableName, Integer versionCount, String rowKey) {
        if (isExists(tableName) && StringUtils.isNotBlank(rowKey)) {
            ...
            try {
                ...
                logger.error("hbase get失败. cluster={} tableName={} rowKey={} exception={}", CLUSTER, tableName, rowKey, ExceptionBus.stackTrace(e));
            } finally {
                closeTable(table);
            }
        }
    ...
    public static Result get(String tableName, Get get) {
        if (isExists(tableName) && get != null) {
            ...
            try {
                ...
                logger.error("hbase get失败. cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
            } finally {
                closeTable(table);
            }
        }
    ...
    public static Result[] get(String tableName, List<Get> getList) {
        if (isExists(tableName) && getList != null && !getList.isEmpty()) {
            ...
            try {
                ...
                logger.error("hbase get失败. cluster={} tableName={} size={} exception={}", CLUSTER, tableName, getList.size(), ExceptionBus.stackTrace(e));
            } finally {
                closeTable(table);
            }
        }
    ...
    public static Result[] gets(String tableName, Integer versionCount, String... rowKey) {
        if (isExists(tableName) && rowKey != null && rowKey.length > 0) {
            ...
            try {
                ...
                logger.error("hbase gets失败. cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
            } finally {
                closeTable(table);
            }
        }
    ...
    public static ResultScanner scan(String tableName, String startRow, String stopRow, FilterList.Operator operator, Filter... filters) {
        ...
        if (table != null && StringUtils.isNotBlank(startRow) && StringUtils.isNotBlank(stopRow)) {
            try {
                ...
                logger.error("hbase scan失败. cluster={} tableName={} startRow={} endRow={} exception={}", CLUSTER, tableName, startRow, stopRow, ExceptionBus.stackTrace(e));
            }
        }
    ...
    public static <T extends HBaseBaseBean> List<T> scan(String tableName, Scan scan, Class<T> clazz) {
        ...
        if (table != null && scan != null && clazz != null) {
            ...
            try {
                ...
                logger.error("hbase scan失败. cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
            } finally {
    ...
    public static <T extends HBaseBaseBean> List<T> scanMaxVersions(String tableName, Scan scan, Class<T> clazz) {
        ...
        if (table != null && scan != null && clazz != null) {
            ...
            try {
                ...
                logger.error("hbase scanMaxVersions失败. cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
            } finally {
                closeResultAndTable(rsScanner, table);
            }
        }
    ...
    public static <T extends HBaseBaseBean> List<T> scanAll(String tableName, Class<T> clazz) {
        ...
        if (table != null && clazz != null) {
            ...
            try {
                ...
                logger.error("hbase scanAll失败. cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
            } finally {
                closeResultAndTable(rsScanner, table);
            }
        }
    ...
    public static boolean isExists(String tableName) {
        ...
        try {
            ...
            logger.error("判断HBase表存在失败. cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
        } finally {
            closeAdmin(admin);
        }
        return isExist;
    ...
    private static Table getTable(String tableName) {
        try {
            ...
            logger.error("HBase getTable失败. cluster={} tableName={} exception={}", CLUSTER, tableName, ExceptionBus.stackTrace(e));
        }
        return null;
    }
    ...
    private static <T extends HBaseBaseBean> List<T> multiCell2Field(Result rs, Class<T> clazz, Map<String, Field> fieldMap) {
        ...
        try {
            ...
            logger.error("HBase multiCell2Field失败. cluster={} exception={}", CLUSTER, ExceptionBus.stackTrace(e));
        }
        return objList;
    }
    ...
    public static <T extends HBaseBaseBean> scala.collection.Iterator<T> hbaseRow2BeanList(scala.collection.Iterator<Tuple2<ImmutableBytesWritable, Result>> it, Class<T> clazz) {
        ...
        try {
            ...
            logger.error("HBase hbaseRow2BeanList失败. cluster={} exception={}", CLUSTER, ExceptionBus.stackTrace(e));
        }
    ...
    public static <T extends HBaseBaseBean> List<T> hbaseMultiVersionRow2BeanList(scala.collection.Iterator<Tuple2<ImmutableBytesWritable, Result>> it, Class<T> clazz) {
        ...
        try {
            ...
            logger.error("HBase hbaseMultiVersionRow2BeanList失败. cluster={} exception={}", CLUSTER, ExceptionBus.stackTrace(e));
        }
        return beanList;
    }
    ...
    public static <T extends HBaseBaseBean> Put convert2Put(T obj, boolean insertEmpty) {
        if (obj != null) {
            try {
                ...
                logger.error("HBase convert2Put失败. cluster={} exception={}", CLUSTER, ExceptionBus.stackTrace(e));
            }
        }
        return null;
    }
