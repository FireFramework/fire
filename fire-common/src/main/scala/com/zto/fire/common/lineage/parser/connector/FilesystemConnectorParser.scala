 ...
 * @since 2.3.8
 */
private[fire] object FilesystemConnectorParser extends ConnectorParser {
  ...
  private[fire] def addDatasource(datasource: Datasource, cluster: String, tableName: String, partitions: JSet[SQLTablePartitions], operation: Operation*): Unit = {
    this.addDatasource(datasource, FilesystemDatasource(datasource.toString, cluster, tableName, partitions, toOperationSet(operation: _*)))
  }
}
 ...
 */
case class FilesystemDatasource(datasource: String, cluster: String, tableName: String,
  ...
  override def equals(obj: Any): Boolean = {
    ...
    val target = obj.asInstanceOf[FilesystemDatasource]
...
object FilesystemDatasource extends SqlToDatasource {
  ...
  def mapDatasource(table: SQLTable): Unit = {
    if (this.isNotMatch("filesystem", table)) return
