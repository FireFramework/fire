 ...
 * @since 2.3.8
 */
private[fire] object UnknownConnectorParser extends ConnectorParser {
  ...
  override def parse(tableIdentifier: TableIdentifier, properties: Map[String, String]): Unit = {
    ...
    val finalConnector = if (noEmpty(connector)) connector else Datasource.UNKNOWN.toString
    this.addDatasource(finalConnector, Operation.CREATE_TABLE)
  }
   ...
   * 集群标识
   */
  def addDatasource(connector: String, operation: Operation *): Unit = {
    if (this.canAdd) this.addDatasource(Datasource.UNKNOWN, UnknownDatasource(Datasource.UNKNOWN.toString, connector, toOperationSet(operation: _*)))
  }
}

/**
 * UnknownDatasource数据源
 *
 * @param datasource
 ...
 */
case class UnknownDatasource(datasource: String, connector: String, var operation: JSet[Operation] = new JHashSet[Operation]) extends DatasourceDesc {
  override def equals(obj: Any): Boolean = {
    if (obj == null || getClass != obj.getClass) return false
    val target = obj.asInstanceOf[UnknownDatasource]
    Objects.equals(datasource, target.datasource) && Objects.equals(connector, target.connector)
  }

  override def hashCode(): Int = Objects.hash(datasource)
}

object UnknownDatasource extends SqlToDatasource {
  ...
  def mapDatasource(table: SQLTable): Unit = {
    UnknownConnectorParser.addDatasource(table.getConnector, table.getOperationType.toSeq: _*)
  }
}
