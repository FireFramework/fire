...
object KafkaUtils extends Logging {
   ...
   * @return
   */
  def getConfByKeyNum(kafkaParams: Map[String, Object] = null,
                      url: String = null,
                      topic: String = null,
                      keyNum: Int = KeyNum._1): (String, String, Map[String, String]) = {
    // 1. 设置producer相关额外参数
    ...
    // 3. 获取kafka集群url
    val confBrokers = KafkaUtils.getBrokers(finalProducerConfig, keyNum)
    val finalBrokers = if (noEmpty(confBrokers)) confBrokers else url
    requireNonEmpty(finalBrokers)(s"kafka broker地址不能为空，请检查keyNum=${keyNum}对应的配置信息")

    (finalBrokers, finalTopic, finalProducerConfig.toMap)
  }
}
