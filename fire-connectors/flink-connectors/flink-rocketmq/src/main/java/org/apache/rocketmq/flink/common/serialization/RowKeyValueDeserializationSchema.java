...
import org.apache.rocketmq.flink.util.ByteSerializer;
import org.apache.rocketmq.flink.util.StringSerializer;
...
/**
 * * The row based implementation of {@link org.apache.rocketmq.flink.common.serialization.KeyValueDeserializationSchema} for the deserialization
 * of message key and value..
 */
public class RowKeyValueDeserializationSchema implements TagKeyValueDeserializationSchema<RowData> {
    ...
    private final TypeInformation<?>[] fieldTypesArray;
    ...
    public RowKeyValueDeserializationSchema(
        ...
        fieldTypesArray = tableSchema.getFieldTypes();
        for (int index = 0; index < columnSize; index++) {
    ...
    public RowData deserializeTagKeyAndValue(MessageExt msg) {
        if (isOnlyHaveVarbinaryDataField()) {
            ...
            return deserializeValue(msg.getBody());
        }
    }
    ...
    private boolean isOnlyHaveVarbinaryDataField() {
        if (columnSize == 1) {
            return isByteArrayType(this.fieldNames[0]);
        }
        return false;
    }
    ...
    private boolean isByteArrayType(String fieldName) {
        TypeInformation<?> typeInformation = fieldTypesArray[columnIndexMapping.get(fieldName)];
    ...
    public static class Builder {
        ...
        public Builder setTableSchema(TableSchema tableSchema) {
            ...
            this.setFieldNames(tableSchema.getFieldNames());
            return this;
        }
