...
　　支持基于接口、apollo、配置文件以及注解等多种方式配置，支持将spark&flink等**引擎参数**、**fire框架参数**以及**用户自定义参数**混合配置，支持运行时动态修改配置。几种常用配置方式如下（[配置手册](./docs/properties.md)）：
...
![fire血缘架构](data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1610 912"></svg>)

​		Fire框架同时支持**Spark**与**Flink**两大流式计算引擎的血缘解析，可实现全面洞悉数据流转链路、打通离线与实时计算的血缘形成闭环、还可协助快速定位线上的问题。		![fire血缘展示](data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1248 832"></svg>)
