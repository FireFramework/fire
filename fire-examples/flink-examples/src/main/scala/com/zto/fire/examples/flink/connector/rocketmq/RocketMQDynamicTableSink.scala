 ...
 */
class RocketMQDynamicTableSink(tableSchema: TableSchema, options: ReadableConfig, dataType: DataType) extends DynamicTableSink {
  override def getChangelogMode(requestedMode: ChangelogMode): ChangelogMode = ChangelogMode.insertOnly()

  override def copy(): DynamicTableSink = new RocketMQDynamicTableSink(tableSchema, options, dataType)
