...
object HBaseOperTest extends BaseSparkCore {
  ...
  /**
    ...
    */
  def testHbaseOperPutList(multiVersion: Boolean = false): Unit = {
    val studentList = Student.newStudentList()
    this.spark.hbaseOperPutList(this.tableName1, JavaConversions.asScalaBuffer(studentList), multiVersion)
  }
  ...
  /**
    ...
    */
  def testHbaseOperPutRDD(multiVersion: Boolean = false): Unit = {
  ...
  /**
    ...
    */
  def testHbaseOperPutDF(multiVersion: Boolean = false): Unit = {
    val studentList = Student.buildStudentList()
    val studentDF = this.spark.createDataFrame(studentList, classOf[Student])
    // 每个批次插100条
    studentDF.hbaseOperPutDF(this.tableName1, classOf[Student], false, 100, multiVersion)
  }
  ...
  /**
    ...
    */
  def testHbaseOperPutDS(multiVersion: Boolean = false): Unit = {
    val studentList = Student.buildStudentList()
    val studentDS = this.spark.createDataset(JavaConversions.asScalaBuffer(studentList))(Encoders.bean(classOf[Student]))
    // 以多版本形式插入
    studentDS.hbaseOperPutDS(this.tableName1, classOf[Student], false, 100, multiVersion)
  }

  /**
    * 使用HBaseOper get数据，并将结果以list方式返回
    */
  def testHbaseOperGetList(multiVersion: Boolean = false): Unit = {
    println("===========testHbaseOperGetList===========")
    val rowKeys = Seq("1", "2", "3", "5", "6")
    val studentList = this.spark.hbaseOperGetList2(this.tableName1, rowKeys, classOf[Student], multiVersion)
    ...
    val studentList2 = this.spark.hbaseOperGetList(this.tableName1, getList, classOf[Student], true)
  ...
  /**
    ...
    */
  def testHbaseOperGetRDD(multiVersion: Boolean = false): Unit = {
    println("===========testHbaseOperGetRDD===========")
    val getList = Seq("1", "2", "3", "5", "6")
    val getRDD = this.spark.parallelize(getList, 2)
    // 以多版本方式get，并将结果集封装到rdd中返回
    val studentRDD = this.spark.hbaseOperGetRDD(this.tableName1, getRDD, classOf[Student], multiVersion)
  ...
  /**
    ...
    */
  def testHbaseOperGetDF(multiVersion: Boolean = false): Unit = {
    println("===========testHbaseOperGetDF===========")
    val getList = Seq("1", "2", "3", "4", "5", "6")
    val getRDD = this.spark.parallelize(getList, 3)
    // get到的结果以dataframe形式返回
    val studentDF = this.spark.hbaseOperGetDF(this.tableName1, getRDD, classOf[Student], multiVersion)
  ...
  /**
    ...
    */
  def testHbaseOperGetDS(multiVersion: Boolean = false): Unit = {
    println("===========testHbaseOperGetDS===========")
    val getList = Seq("1", "2", "3", "4", "5", "6")
    val getRDD = this.spark.parallelize(getList, 2)
    // 指定在多版本获取时只取最新的两个版本
    val studentDS = this.spark.hbaseOperGetDS(this.tableName1, getRDD, classOf[Student], multiVersion, 2)
  ...
  /**
    ...
    */
  def testHbaseOperScanList(multiVersion: Boolean = false): Unit = {
    println("===========testHbaseOperScanList===========")
    val list = this.spark.hbaseOperScanList2(this.tableName1, "1", "6", classOf[Student], multiVersion)
  ...
  /**
    * 使用HBaseOper scan数据，并以DataFrame方式返回
    */
  def testHbaseOperScanDS(multiVersion: Boolean = false): Unit = {
    println("===========testHbaseOperScanDF===========")
    val dataSet = this.spark.hbaseOperScanDS2(this.tableName1, "1", "6", classOf[Student], multiVersion)
  ...
  override def process: Unit = {
    ...
    this.testHbaseOperGetRDD(multiVersion)
    this.testHbaseOperGetDF(multiVersion)
    this.testHbaseOperGetDS(multiVersion)

    println("=========scan========")
    this.testHbaseOperScanList(multiVersion)
    this.testHbaseOperScanRDD(multiVersion)
    this.testHbaseOperScanDF(multiVersion)
  }
