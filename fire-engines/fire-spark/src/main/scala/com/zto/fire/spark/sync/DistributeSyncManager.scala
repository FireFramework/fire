...
object DistributeSyncManager extends SyncManager {
  ...
  def sync(fun: => Unit, isAsync: Boolean = true): Unit = {
    ...
    val activeStages = SparkSingletonFactory.getActiveStageIds.size
    val activeExecutors = this.getInitExecutors
    val rdd = this.sc.parallelize(1 to activeExecutors * 10, activeExecutors * 3)
    this.logInfo(s"当前Active的executor数：$activeExecutors, Active 的stage数：${activeStages}")

    if (activeStages > FireFrameworkConf.lineageActiveStageThreshold) return
    if (isAsync) {
