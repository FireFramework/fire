...
import com.zto.fire.common.conf.{FireFrameworkConf, FireKafkaConf, KeyNum}
 ...
 */
trait AbstractSparkStreaming extends BaseSpark {
  ...
  def init(batchDuration: Long, isCheckPoint: Boolean, conf: SparkConf, args: Array[String]): Unit = {
    ...
    PropUtils.setProperty(FireSparkConf.SPARK_STREAMING_BATCH_DURATION, this.batchDuration.toString)
    if (!isCheckPoint) {
   ...
   */
  @Deprecated
  def kafkaParams(groupId: String = this.appName, kafkaBrokers: String = null, offset: String = FireKafkaConf.offsetLargest, autoCommit: Boolean = false, keyNum: Int = KeyNum._1): Map[String, Object] = {
  ...
  def hotRestart(request: Request, response: Response): AnyRef = {
    ...
    try {
      logInfo(s"Ip address ${request.ip()} request /system/streaming/hotRestart")
      ...
      logInfo(s"[hotRestart] 执行热重启成功：duration=${this.externalConf.getBatchDuration} json=$json")
      ResultMsg.buildSuccess(s"执行热重启成功：duration=${this.externalConf.getBatchDuration}", ErrorCode.SUCCESS.toString)
    } catch {
      case e: Exception => {
        logError(s"[hotRestart] 执行热重启失败：json=$json", e)
